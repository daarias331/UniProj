...
def build_gdal_call(band,infiles, outfile):
    return  'gdal_calc.py -A '+' '.join(infiles)+f' --outfile={outfile} --A_band={band} --calc="numpy.nanmean(A,axis=0)" --NoDataValue=nan'
            

def average_ssp_period(band, ssp, period):
    ...
    if not os.path.exists(input_folder):
        ...
        out_folder=os.path.join(os.getcwd(), f'out_{ssp}_{period}')
...
average_ssp_period(2, "ssp126", "2021-2040")
     
#%%

print(arrBand3[0,0],type(arrBand3[0,0]))
